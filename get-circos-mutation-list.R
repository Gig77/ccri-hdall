...
m <- read.delim("~/hdall/results/impacted-genes-list.tsv")
...
write.table(all[!is.na(all$chr),c("chr", "start", "end", "num_mut")], file="~/hdall/results/mutations-exome.all.circos.tsv", row.names=F, col.names=F, sep="\t", quote=F)
...
write.table(ns[!is.na(ns$num_mut), c("chr", "start", "end", "num_mut")], file="~/hdall/results/mutations-exome.nonsilent.circos.tsv", row.names=F, col.names=F, sep="\t", quote=F)
...
write.table(del[!is.na(del$num_mut),c("chr", "start", "end", "num_mut")], file="~/hdall/results/mutations-exome.deleterious.circos.tsv", row.names=F, col.names=F, sep="\t", quote=F)
