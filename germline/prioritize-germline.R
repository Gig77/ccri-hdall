m <- read.delim("~/hdall/results/germline/filtered-variants.germline.nonsilent.tsv", na.strings=c(""), stringsAsFactors=F)
...
write.table(m[!is.na(m$selected_because),], file="~/hdall/results/germline/filtered-variants.germline.nonsilent.high-priority.tsv", col.names=T, row.names=F, sep="\t", quote=F)