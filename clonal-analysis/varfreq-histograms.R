...
for(p in patients)
{
	d <- data$freq_leu[data$patient==p & data$sample=="rem_dia" & data$dp_leu_tot >= min_cov & data$var_type == "snp"]
...
for(p in patients)
{
	d <- data$freq_leu[data$patient==p & data$sample=="rem_rel" & data$dp_leu_tot >= min_cov & data$var_type == "snp"]
