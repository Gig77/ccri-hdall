...
m <- m[m$dia>=0.1 | m$rel>=0.1 | m$rel3>=0.1,]
...
if (p != "715")
{
	...
	for(i in 1:nrow(m)) {
		...
		frel <- m[i,"rel"]
		frel3 <- m[i,"rel3"]
		lines(c(1.3, 2.8, 4.3), c(fdia, frel, frel3), type="b", col=rgb(fdia, 0, frel, ifelse(m[i, "gene"] %in% genes.to.label, 1, 0.3)), lwd=ifelse(m[i, "gene"] %in% genes.to.label, 4, 1))
