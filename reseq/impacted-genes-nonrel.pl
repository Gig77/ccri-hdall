...
# TABLE: filtered-variants.reseq.nonrel
my %genes;
my $written = 0;
<>; # skip header
while(<>)
{
	chomp;
	my $line = $_;
	my ($patient, $var_type, $status, $rejected_because, $chr, $pos, $dbSNP, $ref, $alt, $gene, $add_genes, $impact, $effect, $non_silent, $deleterious, $exons, 
		$dp_rem_tot, $dp_rem_ref, $dp_rem_var, $freq_rem, $dp_leu_tot, $dp_leu_ref, $dp_leu_var, $freq_leu, $both_strands, $p_value, $aa_change, $snpeff,
		$polyphen2, $sift, $gerp, $siphy, $interpro) = split("\t", $line);

	my $sample = "nonrel-dia";
	$freq_leu = $freq_leu / 100;
