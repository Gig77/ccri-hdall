...
min_cov = 50
patients <- c("C", "B", "460", "818", "592", "446")  # 314, 399 excluded due to low coverage; patients 715, 545, 430 are main figure plots
...
for(p in patients)
{
	...
	for(i in 1:nrow(m.patient)) {
		...
		lines(c(1.3, 2.8), c(fdia, frel), type="b", col=rgb(fdia, 0, frel, ifelse(m.patient[i, "gene"] %in% genes.to.label, 1, 0.2)), lwd=ifelse(m.patient[i, "gene"] %in% genes.to.label, 3, 1))
