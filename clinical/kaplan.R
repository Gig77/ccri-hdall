...
text(60, timepoints(fit.overall, 60)$est[1,]+0.03, sprintf("%.2f, SE=%.2f", timepoints(fit.overall, 60)$est[1,], timepoints(fit.overall, 60)$var[1,]), col="darkgray", adj=0)
box()
...
plot(fit[1:2], xlab="months", ylab="cumulative incidence 2nd relapse", curvlab=c("wtRAS", "mRAS"), col=c("blue", "red"), wh=c(-100,-100), yaxt='n', lty=c(1, 2), xlim=c(0,125))
axis(side=2, at=seq(0, 1, by=0.1))
text(60, timepoints(fit, 60)$est["TRUE 2nd rel",]+0.03, sprintf("%.2f, SE=%.2f", timepoints(fit, 60)$est["TRUE 2nd rel",], timepoints(fit, 60)$var["TRUE 2nd rel",]), col="red", adj=0)
text(60, timepoints(fit, 60)$est["FALSE 2nd rel",]-0.05, sprintf("%.2f, SE=%.2f", timepoints(fit, 60)$est["FALSE 2nd rel",], timepoints(fit, 60)$var["FALSE 2nd rel",]), col="blue", adj=0)
box()
...
plot(fit[1:4], xlab="months", ylab="cumulative incidence 2nd relapse", curvlab=c("mKRAS", "mNRAS", "mPTPN11", "wtRAS"), col=c("red", "blue", "orange", "black"), wh=c(-100,-100), yaxt='n', lty=c(1, 3, 4, 2), xlim=c(0,125))
axis(side=2, at=seq(0, 1, by=0.1))
text(60, timepoints(fit, 60)$est["KRAS 2nd rel",]+0.03, sprintf("%.2f, SE=%.2f", timepoints(fit, 60)$est["KRAS 2nd rel",], timepoints(fit, 60)$var["KRAS 2nd rel",]), col="red", adj=0)
text(60, timepoints(fit, 60)$est["NRAS 2nd rel",]-0.04, sprintf("%.2f, SE=%.2f", timepoints(fit, 60)$est["NRAS 2nd rel",], timepoints(fit, 60)$var["NRAS 2nd rel",]), col="blue", adj=0)
text(60, timepoints(fit, 60)$est["PTPN11 2nd rel",]+0.03, sprintf("%.2f, SE=%.2f", timepoints(fit, 60)$est["PTPN11 2nd rel",], timepoints(fit, 60)$var["PTPN11 2nd rel",]), col="orange", adj=0)
text(60, timepoints(fit, 60)$est["WT 2nd rel",]+0.03, sprintf("%.2f, SE=%.2f", timepoints(fit, 60)$est["WT 2nd rel",], timepoints(fit, 60)$var["WT 2nd rel",]), col="black", adj=0)
box()
legend("topleft", c(sprintf("mKRAS (%d/%d)", sum(complete.cases(c[c$ras.pw.gene.rel=="KRAS" & c$second_event_after_first_relapse=="2nd rel", "second_rem_months"])),  sum(complete.cases(c[c$ras.pw.gene.rel=="KRAS", c("second_rem_months", "second_event_after_first_relapse")]))), 
					...
					lwd=c(1,1), col=c("red", "blue", "orange", "black"), lty=c(1, 3, 4, 2))
text(120, 1, sprintf("p=%.2g", fit$Tests["2nd rel", "pv"]), adj=1)
...
legend("right", c(sprintf("wtCREBBP+wtKRAS (%d/%d)", sum(fit[3]$n.event), fit[3]$n), sprintf("mCREBBP (%d/%d)", sum(fit[1]$n.event), fit[1]$n), sprintf("wtCREBBP+mKRAS (%d/%d)", sum(fit[2]$n.event), fit[2]$n)), lty=c(4, 3, 1), col=c("blue", "black", "red"))
text(1, 0, sprintf("p=%.2g", 1 - pchisq(survdiff(form, data=c)$chisq, length(survdiff(form, data=c)$n) - 1)), adj=0)
...
plot(fit[1:3], xlab="months", ylab="cumulative incidence 2nd relapse", curvlab=c("mCREBBP", "wtCREBBP+mKRAS", "wtCREBBP+wtKRAS"), col=c("black", "red", "blue"), wh=c(-100,-100), yaxt='n', lty=c(3, 1, 4), xlim=c(0,125))
axis(side=2, at=seq(0, 1, by=0.1))
text(60, timepoints(fit, 60)$est["mCREBBP 2nd rel",]+0.03, sprintf("%.2f, SE=%.2f", timepoints(fit, 60)$est["mCREBBP 2nd rel",], timepoints(fit, 60)$var["mCREBBP 2nd rel",]), col="black", adj=0)
text(60, timepoints(fit, 60)$est["wtCREBBP+mKRAS 2nd rel",]-0.04, sprintf("%.2f, SE=%.2f", timepoints(fit, 60)$est["wtCREBBP+mKRAS 2nd rel",], timepoints(fit, 60)$var["wtCREBBP+mKRAS 2nd rel",]), col="red", adj=0)
text(60, timepoints(fit, 60)$est["wtCREBBP+wtKRAS 2nd rel",]+0.03, sprintf("%.2f, SE=%.2f", timepoints(fit, 60)$est["wtCREBBP+wtKRAS 2nd rel",], timepoints(fit, 60)$var["wtCREBBP+wtKRAS 2nd rel",]), col="blue", adj=0)
box()
legend("topleft", c(sprintf("mCREBBP (%d/%d)", sum(complete.cases(c[c$kras.crebbp.rel=="mCREBBP" & c$second_event_after_first_relapse=="2nd rel", "second_rem_months"])),  sum(complete.cases(c[c$kras.crebbp.rel=="mCREBBP", c("second_rem_months", "second_event_after_first_relapse")]))), 
		...
		lwd=c(1,1), col=c("black", "red", "blue"), lty=c(3, 1, 4))
text(120, 1, sprintf("p=%.2g", fit$Tests["2nd rel", "pv"]), adj=1)
